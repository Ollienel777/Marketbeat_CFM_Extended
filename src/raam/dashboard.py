...
page = st.sidebar.radio("View", ["Live strategy", "Backtest comparison"])

runs = list_runs(db_path)

if page == "Live strategy":
    if runs.empty:
...
else:  # Backtest comparison
    st.caption(
        "Compare different strategy/parameter versions. Tag each backtest with `raam-backtest "
        "--label <name>` (e.g. 'v1-baseline', 'v2-ewma-vol') to keep them identifiable here."
    )

    backtests = list_backtest_runs(db_path)
    if backtests.empty:
        st.info("No backtests recorded yet. Run `raam-backtest --tickers <file> --start ... --end ...` first.")
        st.stop()

    backtests = backtests.sort_values("backtest_id", ascending=False)
    backtests["display_label"] = backtests.apply(
        lambda r: f"#{r.backtest_id} — {r.label or '(no label)'} ({r.start_date} to {r.end_date})", axis=1
    )

    selected = st.multiselect(
        "Backtests to compare", backtests["display_label"].tolist(), default=backtests["display_label"].tolist()[:5]
    )
    selected_ids = [int(s.split(" — ")[0].lstrip("#")) for s in selected]

    if not selected_ids:
        st.caption("Select at least one backtest above.")
        st.stop()

    st.subheader("Summary metrics")
    summary_cols = [
        "backtest_id", "label", "start_date", "end_date", "freq",
        "strategy_cagr", "strategy_vol", "strategy_sharpe", "strategy_max_drawdown", "strategy_pct_positive_months",
        "benchmark_ticker", "benchmark_cagr", "benchmark_sharpe",
    ]
    st.dataframe(
        backtests[backtests["backtest_id"].isin(selected_ids)][summary_cols].set_index("backtest_id"),
        use_container_width=True,
    )

    st.subheader("Normalized equity curves (% return from start)")
    st.caption("Each curve is rebased to 100 at its own start, so backtests over different date ranges or budgets can be overlaid on a common 'days since start' axis.")
    ...
    for backtest_id in selected_ids:
        ...
        if curve.empty:
            continue
        label_row = backtests[backtests["backtest_id"] == backtest_id].iloc[0]
        series_label = f"#{backtest_id} {label_row['label'] or ''}".strip()
        normalized = curve["strategy_value"] / curve["strategy_value"].iloc[0] * 100
        combined[series_label] = pd.Series(normalized.values, index=range(len(normalized)))

    if combined:
        chart_df = pd.DataFrame(combined)
        chart_df.index.name = "days_since_start"
        st.line_chart(chart_df)
    else:
        st.caption("No equity curve data found for the selected backtests.")
